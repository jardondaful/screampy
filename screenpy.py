...
# Update the model path to match where you extracted it
model_path = "s2v_old"
print(f"Looking for model in: {os.path.abspath(model_path)}")

if not os.path.exists(model_path):
    print("Model directory not found. Please ensure you've extracted the model correctly.")
    print("Expected path:", os.path.abspath(model_path))
    exit(1)

# Load the sense2vec model
try:
    s2v_model = Sense2Vec().from_disk(model_path)
...
def sense2vec_sim(token1, token2):
    try:
        # Check if both tokens are in the model
        if token1 in s2v_model and token2 in s2v_model:
            # Calculate and return the similarity
            return s2v_model.similarity(token1, token2)
        else:
            return 0
    except Exception as e:
        print(f"Error in sense2vec_sim: {str(e)}")
        return 0
...
def check_hardcode(s):
	try:
		...
		try:
			...
			print('stop word cannot be used for time expression')
			return 0
	return x
...
def is_time(s):
	...
	print(f'is_time({s})')  # Changed Log to print
	if is_date(s):
		return True
...
if __name__ == '__main__':
    try:
        print("Testing sense2vec_sim function:")
        token1 = "python|NOUN"
        token2 = "programming|NOUN"
        print(f"Tokens in model: {token1 in s2v_model}, {token2 in s2v_model}")
        similarity = sense2vec_sim(token1, token2)
        print(f"Similarity between '{token1}' and '{token2}': {similarity}")
        
        print("\nTesting is_time function:")
        print(f"is_time('3 PM'): {is_time('3 PM')}")
        print(f"is_time('tomorrow'): {is_time('tomorrow')}")
        print(f"is_time('elephant'): {is_time('elephant')}")

        print("\nTesting alpha parsing:")
        test_string = "EXT. THE JUNGLE - DAY"
        result = alpha.parseString(test_string)
